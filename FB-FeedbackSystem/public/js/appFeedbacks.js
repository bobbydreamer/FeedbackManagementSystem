document.addEventListener('DOMContentLoaded', event => {
    ...
    const appName = document.getElementById('application').getAttribute("data-application"); 
    ...
    function getCategories(uid){
        return new Promise( (resolve, reject) => {
            ...
            catRef.on('value', function(snap) {
                //console.log('getCategories = ',snap.val());
                if(snap.val() == null) {
                    // return 0;
    ...
    function prepareFeedbacks(){        
            var messagesRef = firebase.database().ref().child('FB_Messages/'+ appName +'/').limitToLast(200);
...
//                        temp['CategoryList'] = catList;
    ...
    firebase.auth().onAuthStateChanged(firebaseUser => {
        if(firebaseUser){
            ...
            Promise.all([promisegetAppList, promisegetCategories]).then(function(results) {
                if(appList[appName] == undefined){
                    document.getElementById('application').innerHTML = 'No feedbacks provided';    
                }else{
                    document.getElementById('application').innerHTML = "Top feedbacks from '"+appList[appName]+"'";
                    prepareFeedbacks();            
                }
            }).catch( (error) => {
   ...
   //                 console.log('temp = ',JSON.stringify(temp));
                ...
                //firebase.database().ref().child('/FB_Categories/CategoryList/' + optValue ).transaction(function (i) { return i+1; });
