document.addEventListener('DOMContentLoaded', event => {
    ...
    var catName = document.getElementsByTagName("body");
    catName = catName[0].getAttribute("data-category");
    //console.log('Category Name = ',catName);
    var colors = [];
    var appList = {};
    var catList = [];
    var categoryDataCount, categoryData;
    ...
    function getCategoryData(uid){
        return new Promise( (resolve, reject) => {
            var archiveRef = firebase.database().ref().child('/FB_Categories/'+uid+'/messages/'+catName+'/');
            archiveRef.on('value', function(snap) {
                if(snap.val() == null) return 0;
                categoryDataCount = snap.numChildren();
                categoryData = snap.val();
                // console.log('categoryDataCount = ',categoryDataCount);
...
//                        temp['CategoryList'] = catList;
                        ...
                        if(allMessages[keys[i]].status == 'active')
                            temp['dis'] = '';
                        else
                            temp['dis'] = 'disabled';    
    ...
    firebase.auth().onAuthStateChanged(firebaseUser => {
        if(firebaseUser){
            ...
            let promisegetCategoryList = getCategoryList(firebaseUser.uid);
            let promisegetCategoryData = getCategoryData(firebaseUser.uid);
            // console.log('promisegetAppList = ',promisegetAppList);
            // console.log('promisegetCategoryList = ',promisegetCategoryList);
            Promise.all([promisegetAppList, promisegetCategoryList, promisegetCategoryData]).then(function(results) {
                if(categoryDataCount > 0 ){
                    document.getElementById('application').innerHTML = "Feedbacks from category '"+catName+"'";
                    showData();            
                }else{
                    document.getElementById('application').innerHTML = 'No feedbacks for this category';    
                }
            }).catch( (error) => {
